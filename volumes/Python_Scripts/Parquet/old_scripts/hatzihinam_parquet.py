...
retailer_source_prefix = "hatzihinam-promofull-gz/"
...
retailer_prefix = "hatzihinam"

# Build SparkSession with Hadoop AWS and spark-xml
spark = (
    SparkSession.builder
    .master("local[2]")  # limit local cores to reduce CPU/memory pressure
    .appName("LoadHatzihinamXMLFromS3")
...
df = (
    ...
    .schema(schema)
    .load(s3_source_path)
    .coalesce(2)  # reduce partitions early
)
...
# Avoid count() to reduce memory usage
...
df_items = (
    ...
    .dropDuplicates(["PromotionID", "ItemCode"])  # lighter than global distinct in many cases
    .coalesce(2)
)
